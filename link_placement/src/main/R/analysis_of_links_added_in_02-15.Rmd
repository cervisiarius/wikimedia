...
# Compute the relative position of the first link occurrence in the article text.
...
n_indir_before <- data$num_paths_before - data$num_wiki_searches_before - data$num_external_searches_before
n_indir_after <- data$num_paths_after - data$num_wiki_searches_after - data$num_external_searches_after - data$num_clicks_after
x <- n_indir_before / data$num_source_before
y <- n_indir_after / data$num_source_after
plot(x, y, col='#00000005', pch=16, log='xy', xlim=c(1e-6, 1), ylim=c(1e-6, 1), cex=2,
     xlab='Fraction paths before', ylab='Fraction paths after (indirect only)')
abline(0,1, col='red')
# This boxplot shows that the prob of indirect paths changes.
groups_idx <- split(1:nrow(data), cut(n_indir_before, breaks=c(1,2,10,50,100,Inf), right=FALSE))
groups_val <- lapply(groups_idx, function(idx) ((y-x)/x)[idx])
...
# Normalized by p_s.
n_search_before <- data$num_wiki_searches_before + data$num_external_searches_before
n_search_after <- data$num_wiki_searches_after + data$num_external_searches_after
x <- n_search_before / data$num_source_before
y <- n_search_after / data$num_source_after
plot(x, y, col='#00000010', pch=16, log='xy', , xlim=c(1e-6, 1e-1), ylim=c(1e-6, 1e-1), cex=2,
     xlab='Search prob before', ylab='Search prob after')
abline(0,1, col='red')
# This boxplot shows that the search prob changes.
groups_idx <- split(1:nrow(data), cut(n_search_before, breaks=c(1,2,10,30,Inf), right=FALSE))
groups_val <- lapply(groups_idx, function(idx) ((y-x)/x)[idx])
boxplot(groups_val, notch=TRUE, names=names(groups_idx), outline=FALSE)
...
```{r}
plot_groups <- function(x, breaks) {
  idx <- which(is.finite(x))
  means <- unlist(lapply(split(data$pst_after[idx], cut(x[idx], breaks=breaks)), function(x) mean(x, na.rm=TRUE)))
  groups <- split(data$pst_after[idx], cut(x[idx], breaks=breaks))
  boxplot(groups, outline=FALSE, ylim=c(0, max(means)))
  lines(means, type='b')
}

plot_groups(data$rel_link_pos, quantile(data$rel_link_pos, seq(0,1,0.1), na.rm=TRUE))
plot_groups(data$outdegree, quantile(data$outdegree, seq(0,1,0.1), na.rm=TRUE))
plot_groups(data$source_length, quantile(data$source_length, seq(0,1,0.1), na.rm=TRUE))
```
...
#######################################################################
##### Does this exclude illegal articles?!?!

x <- sort(abs(null$p_indir - null$p_s_direct_t))
plot(x, (1:sum(is.finite(x)))/sum(is.finite(x)), log='x', type='l', panel.first=grid())

# Only 2% of data points have p_st = 0, so we may ignore them by taking logs
sum(null$p_s_direct_t==0)/nrow(null)
sum(null$p_indir==0)/nrow(null)

plot(null$p_indir, null$p_s_direct_t, log='xy', col='#00000010', pch=16, panel.first=c(abline(0,1)))

x <- sort(abs(log10(null$p_indir) - log10(null$p_s_direct_t)))
plot(x[is.finite(x)], (1:sum(is.finite(x)))/sum(is.finite(x)), type='l', panel.first=grid())
