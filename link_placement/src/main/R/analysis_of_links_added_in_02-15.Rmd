...
boxplot(groups_val, notch=FALSE, names=names(groups_idx), outline=FALSE,
        xlab='Number indirect paths before', ylab='Rel. drop in indirect-path probability')
abline(h=0)
...
boxplot(groups_val, notch=FALSE, names=names(groups_idx), outline=FALSE,
        xlab='Number searches before', ylab='Rel. drop in search probability')
...
if (save_plots) pdf(sprintf('%s/new_link_usage_ccdf.pdf', PLOTDIR), width=3.2, height=2, pointsize=6,
...
if (save_plots) pdf(sprintf('%s/new_link_pst_ccdf_BY-DEGREE.pdf', PLOTDIR), width=3.2, height=2, pointsize=6,
                    family='Helvetica', useDingbats=FALSE)
for (i in 1:(length(Min)-1)) {
  ...
  if (i == 1) {
    plot(sort(data$pst_after[idx]), (length(idx):1)/length(idx), log='xy', type='l', col=i,
         panel.first=grid(), xlim=c(1e-3,1), ylim=c(1e-6, 1),
         xlab='Mininum p_st', ylab='Fraction of new links')
  } else {
    lines(sort(data$pst_after[idx]), (length(idx):1)/length(idx), col=i)
  }
}
legend('topright', legend=sprintf('[%s)', paste(Min[-length(Min)], Min[-1], sep=', ')), bty='n',
       lty=1, col=1:(length(Min)-1))
if (save_plots) dev.off()
...
draw_search_prop_vs_usage_smoothed <- function(x, y, type, col) {
  smooth <- function(y) ksmooth(1:length(y), y, bandwidth=70, kernel='normal')$y
  plot(x, smooth(y), type='l', col=col, xlim=c(1e-6, 0.1), ylim=c(1e-6, 0.1), log='xy',
       #main=sprintf('%s as a predictor of link quality', type),
       xlab='Search proportion', ylab='p_st', panel.first=abline(0, 1, col='gray'))
  #rug(x, col='#00000050')
...
if (save_plots) pdf(sprintf('%s/pindir_vs_pst.pdf', PLOTDIR), width=2, height=2, pointsize=6, family='Helvetica', useDingbats=FALSE)
smooth <- function(y) ksmooth(1:length(y), y, bandwidth=70, kernel='normal')$y
plot(x, smooth(y), type='l', xlim=c(1e-6, .1), ylim=c(1e-6, .1), log='xy',
     xlab='Probability of indirect path', ylab='p_st', panel.first=abline(0, 1, col='gray'))
if (save_plots) dev.off()
```
...
```{r}
plot_groups <- function(x, y, breaks, xlab=NULL, ylab=NULL, errorbars=TRUE) {
  ...
  #lines(means, type='b')
  plot(1:length(means), means, type='b', xaxt='n', bty='n', xlab=xlab, ylab=ylab)
  axis(1, at=1:length(means), labels=names(groups))
  if (errorbars) dispersion(1:length(means), means, se)
}

# Rel link pos vs. p_st.
if (save_plots) pdf(sprintf('%s/pst_vs_rel_link_position.pdf', PLOTDIR), width=3.2, height=2, pointsize=6,
                    family='Helvetica', useDingbats=FALSE)
plot_groups(data$rel_link_pos, data$pst_after, quantile(data$rel_link_pos, seq(0,1,0.1), na.rm=TRUE),
            'Relative link position', 'p_st', errorbars=FALSE)
if (save_plots) dev.off()

# Src outdeg vs. p_st.
if (save_plots) pdf(sprintf('%s/pst_vs_src_outdeg.pdf', PLOTDIR), width=3.2, height=2, pointsize=6,
                    family='Helvetica', useDingbats=FALSE)
plot_groups(data$outdegree, data$pst_after, quantile(data$outdegree, seq(0,1,0.1), na.rm=TRUE),
            'Source outdegree', 'p_st', errorbars=FALSE)
if (save_plots) dev.off()
...
plot_groups(data$num_source_before, data$pst_after, quantile(data$source_length, seq(0,1,0.1), na.rm=TRUE),
            'Source count before addition', 'p_st')
if (save_plots) dev.off()
