...
abline(v=min(which(active$search_prob_wiki_before[ord_wiki]==0)), col='red')
lines(cumsum(active$click_prob_after[ord_ext])/(1:nrow(active)))
abline(v=min(which(active$search_prob_ext_before[ord_ext]==0)))
lines(cumsum(active$click_prob_after[ord_all])/(1:nrow(active)), col='green')
abline(v=min(which(active$search_prob_all_before[ord_all]==0)), col='green')
...
abline(v=min(which(active$search_prob_wiki_before[ord_wiki]==0)))
...
plot(smooth(y_all), type='l', col='green', xlim=c(0,10000),
     ...
     xlab='Rank w.r.t. search proportion (Jan 15)', ylab='Proportion of direct clicks (Mar 15)')
abline(v=min(which(active$search_prob_all_before[ord_all]==0)))
text(2000, 0.85, 'At least one search')
text(8000, 0.85, 'No search')


plot(active$search_prob_wiki_before[ord_wiki], smooth(active$click_prob_after[ord_wiki]), type='l', col='red', ylim=c(0.5,1), xlim=c(0,1))
rug(active$search_prob_wiki_before[ord_wiki], col='red')
lines(active$search_prob_ext_before[ord_ext], smooth(active$click_prob_after[ord_ext]))
plot(active$search_prob_all_before[ord_all], smooth(active$click_prob_after[ord_all]), type='l', col='green', ylim=c(0.45,0.9), xlim=c(0.005,1), log='x')
rug(active$search_prob_all_before[ord_all], col='#00ff0050')
