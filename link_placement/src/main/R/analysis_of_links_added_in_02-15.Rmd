...
hist(data$click_prob_after, breaks=5, xlab='Direct-click ratio', main='')
```
...
#x <- log(null$p_dir_pred[null$p_dir_pred > 0 & null$p_s_direct_t > 0])
#y <- log(null$p_s_direct_t[null$p_dir_pred > 0 & null$p_s_direct_t > 0])
#lm <- lm(y ~ x)

plot(null$p_dir_pred, null$p_s_direct_t, log='xy', xlim=c(1e-6,1), ylim=c(1e-6,1), pch=16, col='#00000010')
abline(0, 1, col='red')
#curve(coef(lm)[1] + x*coef(lm)[2], from add=TRUE)
...
pairs(null[,c('p_s_direct_t', 'p_dir_pred', 'null_model_p', 'p_indir', 'p_indir_pred')], log='xy', pch=16, col='#00000010',
      xlim=c(1e-6,1), ylim=c(1e-6,1))
