...
if (save_plots) pdf(sprintf('%s/new_link_pst_ccdf_BY-DEGREE.pdf', PLOTDIR), width=1.68, height=1.68, pointsize=6,
                    family='Helvetica', useDingbats=FALSE)
...
for (i in 1:(length(Min)-1)) {
  ...
  if (i == 1) {
    plot(sort(data$pst_after[idx]), (length(idx):1)/length(idx), log='xy', type='l', col=i, bty='n',
...
legend('bottomleft', legend=sprintf('[%s)', paste(Min[-length(Min)], Min[-1], sep=', ')), bty='n',
...
#        xlab='Search proportion', ylab='', panel.first=abline(0, 1, col='gray'))
  ...
  plot(x[idx], y[idx], log='xy', col=c('#80808010'), xlim=c(1e-6, .15), ylim=c(1e-6, .15), bty='n',
...
if (save_plots) pdf(sprintf('%s/psearch_vs_pst.pdf', PLOTDIR), width=1.68, height=1.68, pointsize=6, family='Helvetica', useDingbats=FALSE)
...
plot_groups <- function(x, y, breaks, xlab=NULL, ylab=NULL, errorbars=TRUE) {
  ...
  mtext(xlab, side=1, line=2.4)
  mtext(ylab, side=2, line=2.4)
  if (errorbars) dispersion(1:length(means), means, se)
}

# Rel link pos vs. p_st.
if (save_plots) pdf(sprintf('%s/pst_vs_rel_link_position.pdf', PLOTDIR), width=1.68, height=1.68, pointsize=6,
                    family='Helvetica', useDingbats=FALSE)
par(mar=c(3.4, 3.4, 1.1, 0.6))
plot_groups(data$rel_link_pos, data$pst_after, quantile(data$rel_link_pos, seq(0,1,0.1), na.rm=TRUE),
            'Relative link position', expression(italic(p[st])), errorbars=FALSE)
if (save_plots) dev.off()

# Src outdeg vs. p_st.
if (save_plots) pdf(sprintf('%s/pst_vs_src_outdeg.pdf', PLOTDIR), width=1.68, height=1.68, pointsize=6,
                    family='Helvetica', useDingbats=FALSE)
par(mar=c(3.4, 3.4, 1.1, 0.6))
plot_groups(data$outdegree, data$pst_after, quantile(data$outdegree, seq(0,1,0.1), na.rm=TRUE),
            'Source outdegree', expression(italic(p[st])), errorbars=FALSE)
if (save_plots) dev.off()
