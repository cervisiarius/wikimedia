...
public class TreeExtractorReducer implements Reducer<Text, Text, Text, Text> {
	...
	public void reduce(Text uidAndDay, Iterator<Text> pageviewIterator,
		...
		try {
			...
			for (Pageview root : roots) {
				out.collect(new Text(), new Text(root.toString()));
			}
		} catch (Exception e) {
			System.out.format("%s\n", e.getMessage());
		}
	...
	private void test() throws Exception {
		List<Pageview> session = new ArrayList<Pageview>();
		session.add(new Pageview(new JSONObject(
				"{\"dt\":\"2014-12-04T01:00:10\",\"url\":\"a\",\"referer\":\"-\"}")));
		session.add(new Pageview(new JSONObject(
				"{\"dt\":\"2014-12-04T01:00:15\",\"url\":\"c\",\"referer\":\"a\"}")));
		session.add(new Pageview(new JSONObject(
				"{\"dt\":\"2014-12-04T01:00:20\",\"url\":\"b\",\"referer\":\"a\"}")));
		session.add(new Pageview(new JSONObject(
				"{\"dt\":\"2014-12-04T01:00:25\",\"url\":\"a\",\"referer\":\"b\"}")));
		session.add(new Pageview(new JSONObject(
				"{\"dt\":\"2014-12-04T01:00:30\",\"url\":\"b\",\"referer\":\"a\"}")));
		session.add(new Pageview(new JSONObject(
				"{\"dt\":\"2014-12-04T01:00:40\",\"url\":\"c\",\"referer\":\"b\"}")));
		session.add(new Pageview(new JSONObject(
				"{\"dt\":\"2014-12-04T01:00:50\",\"url\":\"d\",\"referer\":\"c\"}")));
