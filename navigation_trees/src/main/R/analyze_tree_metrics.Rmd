...
if (save_plots) pdf(sprintf('%s/tree_properties.pdf', PLOTDIR), width=1.68, height=1.68, pointsize=6,
...
plot(sort(data$size), rev(1:sum(is.finite(data$size)))/sum(is.finite(data$size)), log='xy', type='l', bty='n',
     xlim=c(1,200), ylim=c(1e-5,1), panel.first=grid(), xlab='', ylab='', col=rgb(.9,.6,0))
lines(sort(data$degree_mean), rev(1:sum(is.finite(data$degree_mean)))/sum(is.finite(data$degree_mean)))
mtext('n', side=1, line=2.4)
mtext(expression(paste('Pr[X >= n]')), side=2, line=2.4)
legend('topright', legend=c('X = tree size', 'X = avg. node deg.'),
