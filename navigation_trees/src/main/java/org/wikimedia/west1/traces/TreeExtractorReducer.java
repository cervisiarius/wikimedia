...
public class TreeExtractorReducer extends Reducer<Text, Text, Text, Text> {
	...
	protected boolean isGoodEvent(JSONObject root, boolean isGlobalRoot, Reducer<Text,Text,Text,Text>.Context context,
	...
	protected boolean isGoodTree(JSONObject root, int recursionDepth, Reducer<Text,Text,Text,Text>.Context context, String lang)
	...
	private List<BrowserEvent> filterTrees(List<BrowserEvent> roots, Reducer<Text,Text,Text,Text>.Context context, String lang) {
	...
	public List<BrowserEvent> sequenceToTrees(List<BrowserEvent> events)
	    throws JSONException {
			...
			for (Text eventText : eventsIt) {
				...
				if (++n > maxNumEvents) {
				 context.getCounter(HADOOP_COUNTERS.REDUCE_TOO_MANY_EVENTS).increment(1);
					...
					context.getCounter(HADOOP_COUNTERS.REDUCE_MSEC_EVENT_CONSTRUCTOR).increment(after - before);
					events.add(event);
