...
public class TreeExtractorReducer extends Reducer<Text, Text, NullWritable, Text> {
	...
	// Job config parameter determining if the user agent should be MD5-hashed.
	private static final String CONF_HASH_UA = "org.wikimedia.west1.traces.hashUserAgent";
	...
	private static final Set<String> FIELDS_TO_KEEP_IN_ROOT = new HashSet<String>(Arrays.asList(
	    ...
	    BrowserEvent.JSON_GEOCODED_DATA));
	...
	private static enum HADOOP_COUNTERS {
		...
		// isGoodEvent).
		REDUCE_SKIPPED_SINGLETON, REDUCE_SKIPPED_WIKIMEDIA_REFERER_IN_ROOT, REDUCE_SKIPPED_AMBIGUOUS, REDUCE_TOO_MANY_EVENTS, REDUCE_STACKOVERFLOW,
	...
	public void setup(Reducer<Text, Text, NullWritable, Text>.Context context) {
		if (context == null) {
			...
			hashUserAgent = conf.getBoolean(CONF_HASH_UA, false);
