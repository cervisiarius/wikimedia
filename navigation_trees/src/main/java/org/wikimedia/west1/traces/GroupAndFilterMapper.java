...
public class GroupAndFilterMapper extends Mapper<LongWritable, Group, Text, Text> {
  ...
  // An ad-hoc pattern to catch bots beyond those captured bu the ua_parser library.
  ...
  public void map(LongWritable key, Group value, Context context) throws IOException,
    ...
    try {
      ...
      boolean isArticle = json.getString(JSON_URI_PATH).startsWith("/wiki/");
      boolean isWikiSearch = json.getString(JSON_URI_PATH).equals("/w/index.php")
          && WikiSearch.QUERY_PATTERN.matcher(json.getString(JSON_URI_QUERY)).matches();
      ...
      if (!json.getString(JSON_HTTP_STATUS).equals("200")
          ...
          && !json.getString(JSON_HTTP_STATUS).equals("302")) {
