...
\FOR{all link candidates $(s,t)$ from all paths in $P$}
...
%\begin{algorithm}[t]
%\caption{Basic navigation algorithm}
%% [1] adds line numbering:
%\begin{algorithmic}[1]
%\label{alg:search}
%\INPUT start node $s$; target node $t$; evaluation function $V$
%\OUTPUT sequence of nodes visited
%\STATE push $s$ onto initially empty stack $\mathcal{S}$
%\WHILE{stack $\mathcal{S}$ is not empty}
%  \STATE pop node $u$ from stack $\mathcal{S}$ and remember it as visited
%  \IF{$u = t$}
%    \STATE \textbf{return} sequence of nodes visited
%  \ELSIF{$u$ has not been visited before}
%    \FOR{all neighbors $u'$ of $u$, in order of increasing value according to $V(u'|u,t)$}
%      \STATE push $u'$ onto stack $\mathcal{S}$
%      \IF{$u' = t$}
%	\STATE \textbf{break} \hspace*{1mm}// \textit{make sure $t$ is picked in next iteration}
%      \ENDIF
%    \ENDFOR
%  \ENDIF
%\ENDWHILE
%\STATE \textbf{return} `no path between $s$ and $t$ exists'
%\end{algorithmic}
%\end{algorithm}
